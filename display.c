#include "function_prototypes.h"
#include "definitions.h"
#include "structs.h"

#include <stdio.h>
#include <string.h>

void display_event(event_t* events, int event_count)
{
    char display_choice[MAX_NAME_LEN];

    int spaces[2];
    int i, j;
    int printed;
    printf("Please enter the event name to view the event, or enter 'all' to view all events\n");
    scanf("%s", display_choice);


    /* to display all events */
    if (event_count >= 0)
    {

        if (strcmp(display_choice, "all") == 0)
        {
            printf("Event Name           Event Type Date       Coordinator\n");
            printf("-------------------- ---------- ---------- --------------------\n");

            for (i = 0; i <= event_count; i++)
            {

                printf("%s", events[i].name);
                spaces[0] = MAX_NAME_LEN - strlen(events[i].name);
                for (j = 0; j <= spaces[0]; j++)
                {
                    printf(" ");
                }

                printf("%s", events[i].event_type);
                spaces[0] = 10 - strlen(events[i].event_type);
                for (j = 0; j <= spaces[0]; j++)
                {
                    printf(" ");
                }

                printf("%02d/%02d/%02d", events[i].event_date.day, events[i].event_date.month,
                       events[i].event_date.year);

                printf(" ");

                printf("%s", events[i].coordinator.username);
                spaces[0] = MAX_NAME_LEN - strlen(events[i].coordinator.username);
                for (j = 0; j <= spaces[0]; j++)
                {
                    printf(" ");
                }
            }
        }

            /*for a specific event*/
        else
        {
            for (i = 0; i <= event_count; i++)
            {
                if (strcmp(display_choice, events[i].name) == 0)
                {
                    printf("Event Name           Event Type Date       Coordinator\n");
                    printf("-------------------- ---------- ---------- --------------------\n");

                    printf("%s", events[i].name);
                    spaces[0] = MAX_NAME_LEN - strlen(events[i].name);
                    for (j = 0; j <= spaces[0]; j++)
                    {
                        printf(" ");
                    }

                    printf("%s", events[i].event_type);
                    spaces[0] = 10 - strlen(events[i].event_type);
                    for (j = 0; j <= spaces[0]; j++)
                    {
                        printf(" ");
                    }

                    printf("%02d/%02d/%02d", events[i].event_date.day, events[i].event_date.month,
                           events[i].event_date.year);

                    printf(" ");

                    printf("%s", events[i].coordinator.username);
                    spaces[0] = MAX_NAME_LEN - strlen(events[i].coordinator.username);
                    for (j = 0; j <= spaces[0]; j++)
                    {
                        printf(" ");
                    }

                    printf("\nNumber of Patrons   Number of Staff Event Time Free or Paid\n");
                    printf("------------------- ------------------- ---------- ------------\n");

                    printf("%d", events[i].num_patrons);
                    int par_len = 1;
                    spaces[0] = MAX_NAME_LEN - par_len;
                    for (j = 0; j <= spaces[0]; j++)
                    {
                        printf(" ");
                    }

                    printf("%d", events[i].num_staff);
                    int sta_len = 1;
                    spaces[0] = MAX_NAME_LEN - sta_len;
                    for (j = 0; j <= spaces[0]; j++)
                    {
                        printf(" ");
                    }

                    printf("%02d:%02d      ", events[i].event_date.hour, events[i].event_date.minute);

                    if (events[i].free_event == 1)
                    {
                        printf("Yes");
                    } else if (events[i].free_event == 0)
                    {
                        printf("No");
                    }

                    printed = 1;
                } else if (i == event_count && printed == 0)
                {
                    printf("No events found");
                }
            }

        }
    }


        /* if no events can be found */
    else
    {
        printf("No events found");
    }
}
